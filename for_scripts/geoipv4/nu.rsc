:global AddressList
/ip firewall address-list
:do {add list=$AddressList comment=nu address=104.28.12.83/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.12.84/31} on-error {}
:do {add list=$AddressList comment=nu address=104.28.125.78/31} on-error {}
:do {add list=$AddressList comment=nu address=104.28.177.84/30} on-error {}
:do {add list=$AddressList comment=nu address=104.28.177.88/31} on-error {}
:do {add list=$AddressList comment=nu address=104.28.209.119/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.209.120/30} on-error {}
:do {add list=$AddressList comment=nu address=104.28.209.124/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.241.119/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.241.120/30} on-error {}
:do {add list=$AddressList comment=nu address=104.28.241.124/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.29.71/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.29.72/32} on-error {}
:do {add list=$AddressList comment=nu address=104.28.35.78/31} on-error {}
:do {add list=$AddressList comment=nu address=104.28.72.20/31} on-error {}
:do {add list=$AddressList comment=nu address=104.28.90.74/31} on-error {}
:do {add list=$AddressList comment=nu address=14.1.76.0/23} on-error {}
:do {add list=$AddressList comment=nu address=140.248.56.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.57.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.58.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.59.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.60.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.61.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.62.169/32} on-error {}
:do {add list=$AddressList comment=nu address=140.248.63.169/32} on-error {}
:do {add list=$AddressList comment=nu address=146.75.132.106/31} on-error {}
:do {add list=$AddressList comment=nu address=146.75.160.106/31} on-error {}
:do {add list=$AddressList comment=nu address=146.75.190.8/31} on-error {}
:do {add list=$AddressList comment=nu address=146.75.213.0/31} on-error {}
:do {add list=$AddressList comment=nu address=162.120.204.56/32} on-error {}
:do {add list=$AddressList comment=nu address=162.120.228.72/32} on-error {}
:do {add list=$AddressList comment=nu address=172.225.175.176/28} on-error {}
:do {add list=$AddressList comment=nu address=172.225.230.208/28} on-error {}
:do {add list=$AddressList comment=nu address=172.225.244.144/28} on-error {}
:do {add list=$AddressList comment=nu address=172.225.65.160/27} on-error {}
:do {add list=$AddressList comment=nu address=194.50.99.171/32} on-error {}
:do {add list=$AddressList comment=nu address=198.54.100.92/31} on-error {}
:do {add list=$AddressList comment=nu address=45.138.10.20/30} on-error {}
:do {add list=$AddressList comment=nu address=5.175.190.136/29} on-error {}
:do {add list=$AddressList comment=nu address=5.230.71.237/32} on-error {}
