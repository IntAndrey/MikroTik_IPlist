:global AddressList
/ip firewall address-list
:do {add list=$AddressList comment=AS401776 address=104.234.67.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=107.149.17.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=107.149.36.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=109.110.162.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=151.243.179.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=151.246.185.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=154.16.52.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=163.5.38.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=163.8.195.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=176.102.174.0/23} on-error {}
:do {add list=$AddressList comment=AS401776 address=178.83.71.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=189.13.147.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=200.180.164.0/23} on-error {}
:do {add list=$AddressList comment=AS401776 address=23.185.104.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=40.223.84.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=45.140.209.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=77.246.220.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=82.24.203.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=82.39.210.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=87.82.192.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=87.83.195.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=87.83.60.0/24} on-error {}
:do {add list=$AddressList comment=AS401776 address=92.119.35.0/24} on-error {}
