:global AddressList
/ip firewall address-list
:do {add list=$AddressList comment=AS216472 address=109.224.242.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=128.0.7.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=131.222.247.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=131.222.250.0/23} on-error {}
:do {add list=$AddressList comment=AS216472 address=131.222.252.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=139.28.48.0/23} on-error {}
:do {add list=$AddressList comment=AS216472 address=164.138.205.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=176.117.104.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=185.254.55.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=188.132.150.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=188.132.249.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=193.177.241.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=195.62.50.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=212.108.115.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=212.68.59.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=85.189.13.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=85.189.9.0/24} on-error {}
:do {add list=$AddressList comment=AS216472 address=89.43.132.0/22} on-error {}
