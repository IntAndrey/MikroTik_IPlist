:global Distance
:global RouteTab
:global GateWay
/ip route
:if ([:len [/ip/route/find dst-address=94.74.156.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.156.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.158.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.158.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.160.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.160.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.168.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.168.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.170.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.170.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.172.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.172.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.176.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.176.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.186.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.186.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=94.74.188.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=94.74.188.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.128.155.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.128.155.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.128.159.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.128.159.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.128.194.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.128.194.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.130.225.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.130.225.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.130.240.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.130.240.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.130.56.0/21 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.130.56.0/21 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.142.224.0/20 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.142.224.0/20 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.216.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.216.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.222.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.222.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.233.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.233.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.234.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.234.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.236.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.236.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.248.0/23 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.248.0/23 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.156.252.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.156.252.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.162.0.0/16 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.162.0.0/16 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.215.160.0/22 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.215.160.0/22 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.215.173.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.215.173.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.215.59.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.215.59.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.38.0.0/16 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.38.0.0/16 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.64.0.0/17 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.64.0.0/17 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.80.128.0/18 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.80.128.0/18 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.81.77.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.81.77.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
:if ([:len [/ip/route/find dst-address=95.81.78.0/24 and gateway=$GateWay and routing-table=$RouteTab]] = 0) do={ add dst-address=95.81.78.0/24 gateway=$GateWay routing-table=$RouteTab distance=$Distance comment=ir }
